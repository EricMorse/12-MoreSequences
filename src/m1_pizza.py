...
def run_test_draw_points_on_circle():
    ...
    # ------------------------------------------------------------------
    title = 'test 4: 4 yellow dots.'
    window = rg.RoseWindow(400, 400, title)
    circle = rg.Circle(rg.Point(250, 150), 50)
...
def run_test_pizza():
    ...
    # ------------------------------------------------------------------
    title = 'PIZZA test 1:  16 slices, thin (thickness=1) black lines.'
    window = rg.RoseWindow(600, 600, title)
    circle = rg.Circle(rg.Point(200, 200), 250)
    circle.outline_thickness = 1
    pizza(window, circle, 16, 'black', 1)
    window.close_on_mouse_click()
...
def run_test_polygon():
    ...
    # ------------------------------------------------------------------
    title = 'POLYGON tests 3: 7 segments with thn green lines.'
    window = rg.RoseWindow(600, 600, title)
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        lines = rg.Line(points[k], points[(k + 1) % number_of_segments])
        lines.color = color
        lines.thickness = thickness
        lines.attach_to(window)
    window.render()
...
def run_test_fancy_polygon():
    ...
    circle.fill_color = 'blue'
    fancy_polygon(window, circle, 30, 7, 'lime green', 5)
    window.close_on_mouse_click()
